...
### Step 4: Constructing Matrix {d}V{d}

In our previous steps, we found our two eigenvalues and their corresponding normalized eigenvectors:
* For {d}\lambda_1 = 6{d}, our vector is {d}v_1 = \begin{{pmatrix}} 1/\sqrt{{2}} \\ 1/\sqrt{{2}} \end{{pmatrix}}{d}
* For {d}\lambda_2 = 2{d}, our vector is {d}v_2 = \begin{{pmatrix}} 1/\sqrt{{2}} \\ -1/\sqrt{{2}} \end{{pmatrix}}{d}

To build the matrix {d}V{d}, we place these vectors as columns. **Crucially, they must be ordered from the largest eigenvalue to the smallest.** Since {d}6 > 2{d}, {d}v_1{d} becomes our first column and {d}v_2{d} becomes our second.

{dd}V = \begin{{pmatrix}} \frac{{1}}{{\sqrt{{2}}}} & \frac{{1}}{{\sqrt{{2}}}} \\ \frac{{1}}{{\sqrt{{2}}}} & -\frac{{1}}{{\sqrt{{2}}}} \end{{pmatrix}}{dd}


### Step 5: Applying the Truncation ({d}V_k{d})

Our original dataset had 2 features (since {d}M{d} was {d}2 \times 2{d}). Let's say we want to reduce our feature space from 2 dimensions down to 1 dimension. This means we set our hyperparameter {d}k = 1{d}.

To truncate {d}V{d} into {d}V_k{d}, we simply keep the first {d}k{d} columns and discard the rest. By keeping only the first column, we retain the vector associated with the largest eigenvalue (the one that captures the most variance in the data).

{dd}V_1 = \begin{{pmatrix}} \frac{{1}}{{\sqrt{{2}}}} \\ \frac{{1}}{{\sqrt{{2}}}} \end{{pmatrix}}{dd}

We have successfully truncated our right singular matrix!

### Step 6: Projecting the Data ({d}X_{{reduced}}{d})

Now we project a hypothetical dataset into this new 1-dimensional space. Let's assume our original data matrix {d}X{d} consists of 3 samples (rows) and 2 features (columns):

{dd}X = \begin{{pmatrix}} 1 & 2 \\ 3 & 4 \\ 5 & 6 \end{{pmatrix}}{dd}

To find our reduced matrix, we compute the dot product of {d}X{d} and our truncated matrix {d}V_1{d}:

{dd}X_{{reduced}} = X V_1{dd}

Let's do the matrix multiplication. We multiply the rows of {d}X{d} by the single column of {d}V_1{d}:

{dd}X_{{reduced}} = \begin{{pmatrix}} 1 & 2 \\ 3 & 4 \\ 5 & 6 \end{{pmatrix}} \begin{{pmatrix}} \frac{{1}}{{\sqrt{{2}}}} \\ \frac{{1}}{{\sqrt{{2}}}} \end{{pmatrix}}{dd}

* **Row 1:** {d}(1 \cdot \frac{{1}}{{\sqrt{{2}}}}) + (2 \cdot \frac{{1}}{{\sqrt{{2}}}}) = \frac{{3}}{{\sqrt{{2}}}}{d}
* **Row 2:** {d}(3 \cdot \frac{{1}}{{\sqrt{{2}}}}) + (4 \cdot \frac{{1}}{{\sqrt{{2}}}}) = \frac{{7}}{{\sqrt{{2}}}}{d}
* **Row 3:** {d}(5 \cdot \frac{{1}}{{\sqrt{{2}}}}) + (6 \cdot \frac{{1}}{{\sqrt{{2}}}}) = \frac{{11}}{{\sqrt{{2}}}}{d}


Putting it all together, our final reduced data matrix is:

{dd}X_{{reduced}} = \begin{{pmatrix}} \frac{{3}}{{\sqrt{{2}}}} \\ \frac{{7}}{{\sqrt{{2}}}} \\ \frac{{11}}{{\sqrt{{2}}}} \end{{pmatrix}}{dd}

#### The Result
We started with a dataset that had 3 samples and **2 features** (a {d}3 \times 2{d} matrix). After finding the eigenvalues, constructing {d}V{d}, truncating to {d}k=1{d}, and computing the dot product, we now have a dataset with 3 samples and only **1 feature** (a {d}3 \times 1{d} matrix). 

You have successfully performed dimensionality reduction by hand using the mathematical foundations of SVD!
"""

    file_name = "README_svd_3.md"
